...
# Path persistence for both Bash and Zsh configurations
for config in ~/.bashrc ~/.zshrc; do
    if [ -f "$config" ]; then
        grep -q "GOPATH" "$config" || echo 'export GOPATH=$HOME/go' >> "$config"
        grep -q "usr/local/go/bin" "$config" || echo 'export PATH=$PATH:/usr/local/go/bin:$HOME/go/bin' >> "$config"
    fi
done
...
mkdir -p ~/.config/subfinder ~/.config/puredns ~/wordlists ~/tools
...
if [ ! -f "$HOME/go/bin/aquatone" ]; then
...
echo -e "${BLUE}[*] =================================================== ${NC}"
EOF
sudo bash /tmp/clean_install.sh
